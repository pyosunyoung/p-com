...
export default function MessageList({id}: Props) {
  ...
  const setGoDown = useMessageStore().setGoDown; //zustand
  ...
  const {
    ...
    queryFn: getMessages,
    initialPageParam: 0,
    getPreviousPageParam: (firstPage) => firstPage.length < 10 ? undefined : firstPage.at(0)?.messageId, // 리버스 인피니트 스크롤링(과거 메시지는 위로 올리니)
  ...
  useEffect(() => { // 스크롤 위로 올렸을 때 데이터 천천히 가져오기? 
    if (inView) {
  ...
  useEffect(() => {
    if (shouldGoDown) { // 새로운 메시지가 올때 스크롤이 바로 밑으로 가게 설정.
      if (listRef.current) {
        listRef.current.scrollTop = listRef.current?.scrollHeight;
        setGoDown(false);
      }
