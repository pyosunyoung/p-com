...
export default async function ChatRoom({ params }: Props) {
  ...
  const ids = params.room.split('-').filter((v) => v !== session?.user?.email); //ids[0]은 상대방 Id 가져옴.
  if (!ids[0]) { //상대방 아이디가 없으면 null
    return null;
  }
  await queryClient.prefetchQuery({queryKey: ['users', ids[0]], queryFn: getUserServer}) //유저 정보 가져옴?
