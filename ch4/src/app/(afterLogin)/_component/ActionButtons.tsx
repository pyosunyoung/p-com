...
export default function ActionButtons({ white, post }: Props) {
  ...
  const heart = useMutation({
    ...
    onMutate() { // heart누른 게시물 찾기
      const queryCache = queryClient.getQueryCache() //react query dev tool에서 보는 값들
      const queryKeys = queryCache.getAll().map(cache => cache.queryKey) //쿼리키를 전부 가져옴
      console.log('queryKeys', queryKeys);//['posts', 'recommends']
        ...
        if (queryKey[0] === 'posts') {
          ...
          if (value && 'pages' in value) {
            ...
            if (obj) { // 존재는 하는지
              ...
              const index = value.pages[pageIndex].findIndex((v) => v.postId === postId); //찾고자 하는 게시글이 있는지
    ...
    onSettled() {
      // queryClient.invalidateQueries({ //전체 게시글 새로고침 할 때 사용
      //   queryKey: ['posts']
      // })
    },
  });

  const unheart = useMutation({ //하트 제거
    mutationFn: () => {
      return fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/posts/${postId}/heart`, {
        method: 'delete',
        credentials: 'include',
    ...
    async onSuccess(response) {
      ...
      queryKeys.forEach((queryKey) => {
        if (queryKey[0] === 'posts') {
          ...
          if (value && 'pages' in value) {
            console.log('array', value);
            const obj = value.pages.flat().find((v) => v.postId === postId); // flat은 2차원 배열을 1차원 배열로 바꿔줌
            if (obj) { // 존재는 하는지
              const pageIndex = value.pages.findIndex((page) => page.includes(obj));
              const index = value.pages[pageIndex].findIndex((v) => v.postId === postId);
              console.log('found index', index);
  ...
  const onClickComment: MouseEventHandler<HTMLButtonElement> = (e) => {
    ...
    modalStore.setData(post); //compost tweet로 가기전에 내가 현재 답글 달기를 진행중이라는 것을 알려줌.
  ...
  const onClickHeart: MouseEventHandler<HTMLButtonElement> = (e) => {
    e.stopPropagation(); // 이렇게 설정하면 하트가 눌려짐
    if (liked) { //이미 좋아요 눌렀으면 하트 제겨ㅓ
      unheart.mutate();
    } else {
      heart.mutate();
    }
